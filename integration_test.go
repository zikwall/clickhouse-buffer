...
import (
	...
	"github.com/zikwall/clickhouse-buffer/v2/src/database/cxsql"
)
...
func useCommonClient(ctx context.Context, ch cx.Clickhouse) cx.Client {
	return cx.NewClientWithOptions(ctx, ch,
		cx.DefaultOptions().SetFlushInterval(500).SetBatchSize(6),
	)
}
...
func useWriteAPI(client cx.Client, buf cx.Buffer) cx.Writer {
	writeAPI := client.Writer(cx.NewView(integrationTableName, []string{"id", "uuid", "insert_ts"}), buf)
	return writeAPI
}

func writeDataToBuffer(writeAPI cx.Writer) {
