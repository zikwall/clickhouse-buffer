...
func (r *retryImpl) action(ctx context.Context, view cx.View, btc *cx.Batch) retry.Action {
...
func (r *retryImpl) resend(packet *Packet, err error) bool {
	if (packet.tryCount < defaultCycloCount) && cx.IsResendAvailable(err) {
		r.Retry(&Packet{
			view:     packet.view,
			batch:    packet.batch,
